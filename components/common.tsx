...
export const Avatar: React.FC<{ size?: 'sm' | 'md' | 'lg', className?: string }> = ({ size = 'md', className = '' }) => {
    ...
    return (
        <div 
            className={`rounded-full bg-center bg-cover bg-[#233045] border border-[#232a36] flex-shrink-0 ${sizeClasses[size]} ${className}`}
...
export const Chip: React.FC<{ children: React.ReactNode, onClick?: () => void, className?: string }> = ({ children, onClick, className }) => (
    <div 
        className={`bg-[#1f2430] text-[#cbd5e1] border border-[#232a36] py-1.5 px-2.5 rounded-full text-xs whitespace-nowrap cursor-pointer hover:bg-[#262c38] ${className}`}
        onClick={onClick}
    >
        {children}
    </div>
...
export const Button: React.FC<ButtonProps> = ({ children, variant = 'secondary', className = '', ...props }) => {
    ...
    const variantClasses = {
        primary: 'bg-[#ff7a00] border border-[#a74800] text-[#18130a] font-bold hover:bg-orange-500',
        secondary: 'bg-[#1a2130] border border-[#232a36] text-[#e6e6e6] hover:bg-[#222b3f]',
...
export const Panel: React.FC<{ children: React.ReactNode, className?: string }> = ({ children, className = '' }) => (
    <div className={`bg-[#141821] border border-[#232a36] rounded-[14px] shadow-[0_10px_30px_rgba(0,0,0,.35)] ${className}`}>
        {children}
    </div>
);
