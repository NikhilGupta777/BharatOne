  ...
  return (
    <header className="sticky top-0 z-50 grid items-center gap-4 border-b border-border-color bg-opacity-75 bg-panel-2 p-3 backdrop-blur-md md:grid-cols-[220px_1fr_auto] md:px-4">
      <div className="brand flex items-center gap-2.5">
        <div className="logo h-9 w-9 rounded-[10px] bg-[radial-gradient(120%_150%_at_0%_0%,#ffd166_0%,#ff7a00_35%,#c2410c_75%)] shadow-[inset_0_0_0_2px_rgba(255,255,255,.08)] shadow-custom"></div>
      ...
      <div className="actions flex items-center justify-end gap-2.5">
        ...
        <Button onClick={onNotifsClick} variant="secondary" className="relative p-2.5">
          <BellIcon />
          {notifsCount > 0 && (
            <div className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-600 text-[10px] font-bold text-white border-2 border-panel-2">{notifsCount}</div>
          )}
        </Button>
